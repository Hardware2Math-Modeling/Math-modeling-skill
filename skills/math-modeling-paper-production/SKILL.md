...
Automatic structure/content-stream checks never authorize visual pass. `produce_paper(...)` does not accept pre-compile visual evidence: it publishes an immutable candidate PDF, an immutable `paper_manifest.json` whose `submission_ready` is false, and an immutable `visual_review_request.json` bound to the manifest/PDF hashes and exact page count. Its readiness record names `paper_finalization.json` as the sole authority.

After compilation, call `render_paper_pages(project_root, iteration, renderer=absolute_executable)` with an explicitly supplied supported renderer; it does not search PATH, install tools, or invoke a shell. The API revalidates the immutable candidate manifest/PDF/request before and after rendering, captures renderer executable hash/version, fixed command, exit code, and log hash, and publishes exactly one project-contained regular PNG per page plus the canonical `paper_render_manifest.json`. Arbitrary bytes, caller-authored render JSON, malformed or blank/transparent PNGs, missing/duplicate pages, dimension mismatches, and stale hashes fail closed.

Record a strict human-review manifest bound to the exact PDF and canonical render-manifest hash; the reviewer supplies no render-manifest path or renderer claims. It must cover every page, identify a nonempty reviewer and UTC review time, and explicitly pass the complete checklist for blank pages, cropping, garbled text, overlap, and abnormal font or hidden padding. Then call `finalize_paper(project_root, iteration, review_path)`. Finalization never recompiles or overwrites; it revalidates the active/non-stale iteration, Gate 3, current validation, candidate manifest/PDF/request, template hashes and eligibility, page/reference/structure gate, production generator and renderer provenance, canonical render manifest, every PNG, and human review before creating `paper_finalization.json`. Missing, unavailable, invalid, incomplete, or stale evidence remains `needs_review` and cannot create readiness.

If candidate publication is interrupted between the immutable manifest and review request, call `recover_paper_publication(project_root, iteration)`. Recovery reconciles exact candidate/PDF/readiness hashes, creates only the missing fixed-path request, never recompiles or overwrites, and rejects mismatched or tampered orphans.
