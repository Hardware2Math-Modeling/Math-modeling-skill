...
Return an updated handoff with `state.stage` set to `model-construction` and a justified `complete` or `needs_revision` status. In `result`, preserve equations, symbols, units, domains, objectives, constraints, assumptions, candidate comparison, accepted and rejected models, construction checks, solution interface, and planned validation tests. In `next`, list unresolved inputs and recommend solution only when the specification is executable; the orchestrator owns routing.
