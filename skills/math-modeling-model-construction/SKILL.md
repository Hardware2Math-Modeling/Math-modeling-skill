...
- Prepare the Gate 2 handoff evidence for each question with `question_id`, `model_id`, `baseline`, `parameter_sources`, `validation_plan` (`validation_cycle_id`, `threshold`, `split`, `scope`, `seed`, and `method`), `acceptable_failure_conditions`, and the project-relative specification artifact path and SHA-256 hash.
